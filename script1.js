document.addEventListener("DOMContentLoaded", () => {
    ...
    function addItemToCart(productName, productAmount) {
        const existingCartItem = [...cartItemsContainer.children].find(item => item.querySelector('.product-name').textContent === productName);
    ...
    document.querySelectorAll('.btn').forEach(button => {
        button.addEventListener('click', (e) => {
            const productName = e.target.closest('.card').getAttribute('data-product-name');
            const productAmount = parseFloat(e.target.closest('.card').getAttribute('data-product-amount'));
